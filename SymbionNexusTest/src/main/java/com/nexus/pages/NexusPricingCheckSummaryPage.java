...
public class NexusPricingCheckSummaryPage extends NexusTestBase {
	...
	public NexusPricingCheckSummaryPage() {
		PageFactory.initElements(driver, this);
	}
	
	public NexusPricingMgrSummaryPage pricingCheckSummaryDetails() throws InterruptedException {
		...
		Alert pricingCheckAlert = driver.switchTo().alert();
	
		String pricingCheckAlertText = pricingCheckAlert.getText();
/*		String[] alertTextArr = alertText.split(" has been");
		bar= alertTextArr[0];
		System.out.println(alertTextArr[0]);
*/		
		System.out.println("Pop up message After sumbisson to Pricing Check : "+pricingCheckAlertText);
		
		Thread.sleep(3000);

		pricingCheckAlert.accept();/* Clicking Ok on Popup*/
		Thread.sleep(2000);
		...
		pricingCheckAlert.accept();
		
		Thread.sleep(2000);
		
		return new NexusPricingMgrSummaryPage();
	}
